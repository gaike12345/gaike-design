...
export const ACCENTS: Record<string, AccentColors> = {
  novel: { main: '#6366F1', light: '#818CF8', bg50: '#EEF2FF', bg100: '#E0E7FF', bg200: '#C7D2FE' },
  ...
  comic: { main: '#7C3AED', light: '#8B5CF6', bg50: '#F5F3FF', bg100: '#EDE9FE', bg200: '#DDD6FE' },
}
...
export interface FeatureLandingProps {
  ...
  preview: { title: string; custom?: ReactNode }
...
export default function FeatureLanding(props: FeatureLandingProps) {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-white">
      ...
      {/* Hero */}
      <section className="container-page flex flex-1 flex-col py-14 sm:py-20">
        <div className="text-xs font-semibold uppercase tracking-wider" style={{ color: a.main, opacity: 0.8 }}>
          {props.tag}
        </div>
        <div className="mt-8 grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-12">
          ...
          <div>
            <h1 className="text-4xl font-bold leading-tight tracking-tight text-slate-900 sm:text-5xl">
              {props.heroTitle.prefix && (
                <span className="text-slate-900/70">{props.heroTitle.prefix}</span>
              )}{' '}
              <span className="bg-clip-text text-transparent" style={{ backgroundImage: grad }}>
                {props.heroTitle.highlight}
              </span>
              {props.heroTitle.suffix && (
                <span className="text-slate-900/70">{props.heroTitle.suffix}</span>
              )}
            </h1>
            ...
            <div className="mt-8 flex flex-col items-start gap-3 sm:flex-row">
              ...
              {props.secondaryCta && (
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition-all hover:shadow-sm"
          ...
          <div className="relative">
            <div
              className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-slate-900 shadow-xl"
            ...
            >
              <div className="flex items-center gap-1.5 border-b border-white/10 px-3 py-2">
                ...
                <div className="ml-2 rounded-md bg-white/5 px-3 py-0.5 text-[11px] text-slate-400">
                  {props.preview.title}
                </div>
              </div>
              <div className="flex h-[calc(100%-34px)] items-center justify-center">
      ...
      {/* 核心能力 */}
      <section className="py-20" style={{ backgroundColor: a.bg50 }}>
        <div className="container-page">
          <h2 className="text-center text-2xl font-bold text-slate-900 sm:text-3xl">核心能力</h2>
          <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {props.abilities.map((ab) => (
              ...
              >
                ...
                <h3 className="mt-5 text-base font-semibold text-slate-900">{ab.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-500">{ab.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-20">
        <div className="container-page">
          <h2 className="text-center text-2xl font-bold text-slate-900 sm:text-3xl">创作流程</h2>
            ...
            {props.steps.map((s, i) => {
              ...
              return (
                <div key={s.title} className="relative flex flex-col items-center text-center">
                  ...
                  <p className="mt-2 max-w-xs text-sm leading-relaxed text-slate-500">{s.desc}</p>
                </div>
              )
            })}
          </div>
        ...
        <section className="py-20 bg-white">
          <div className="container-page">
              ...
              {props.works.map((w) => {
                ...
                return (
                  <Link
                    key={w.title}
                    to={`/community${qs}`}
                    className="group block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_20px_50px_-12px_rgba(15,23,42,0.15)]"
                  >
                    {/* 封面 4:3 */}
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="truncate text-xs text-slate-500">{w.author}</span>
                        </div>
                        <span
            ...
            <div className="mt-10 text-center">
              <Link
                to="/community"
                className="inline-flex items-center gap-2 rounded-xl border bg-white px-6 py-2.5 text-sm font-semibold text-slate-700 transition-all hover:shadow-sm hover:text-slate-900"
                style={{ borderColor: a.bg200 }}
              >
                查看更多作品
                <ArrowRight className="h-4 w-4" />
      ...
      <section className="py-20" style={{ backgroundColor: a.bg50 }}>
        <div className="container-page">
          <div className="relative overflow-hidden rounded-3xl border p-10 text-center sm:p-14"
            ...
            <h2 className="text-2xl font-bold text-slate-900 sm:text-3xl">{props.finalCta.title}</h2>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-slate-600">
              {props.finalCta.desc}
            </p>
            <div className="mt-8 flex justify-center">
              <Link
                to={props.finalCta.button.to}

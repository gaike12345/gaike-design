import { useState, useEffect } from 'react'; import { Link } from 'react-router-dom'; import { FaPlus, FaEdit, FaTrash, FaBlog } from 'react-icons/fa'; import { api } from '../api';
export default function BlogList() {
  ...
  const del = async (id: number) => { if(!confirm('删除？'))return; try{await api.delete(`/blog/${id}`);setPosts(p=>p.filter(x=>x.id!==id));}catch{alert('失败');} };
  return (

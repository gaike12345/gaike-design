import { useState, useEffect } from 'react'; import { FaEnvelope, FaCheck, FaTrash } from 'react-icons/fa'; import { api } from '../api';
export default function ContactsList() {
  ...
  const del = async (id: number) => { if(!confirm('删除？'))return; try{await api.delete(`/contact/${id}`);setContacts(c=>c.filter(x=>x.id!==id));}catch{alert('失败');} };
  return (

...
import { FaCalendar, FaClock, FaCheck, FaUser, FaPhone } from 'react-icons/fa';
...
export default function BookingSection() {
        ...
        if (data.length > 0) {
          const mapped = data
            .map((s) => ({
              ...
              price: cleanPrice(s.price || s.price_text || '¥500/小时'),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('预约成功！我们将尽快通过微信与您联系确认');
      setSelectedDate('');
      setSelectedTime('');
      setSelectedService('');
      setNotes('');
      ...
      toast.error('预约失败，请稍后重试或直接加微信联系');
    } finally {
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-[#0A0A0F] to-[#0F0F1A]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl sm:text-5xl font-bold text-white mb-6">在线预约</h2>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-white font-semibold mb-2">
                  <FaUser className="inline mr-2 text-[#00F5FF]" />
              ...
              <div>
                <label className="block text-white font-semibold mb-2">
                  <FaPhone className="inline mr-2 text-[#00F5FF]" />
                  手机号 / 微信号 <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={contactPhone}
            ...
            {/* 服务 */}
            <div>
              <label className="block text-white font-semibold mb-4">选择服务类型</label>
                ...
                {serviceTypes.map((service) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-[#00F5FF] font-semibold">{service.price}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {/* 备注 */}
            <div>
              <label className="block text-white font-semibold mb-2">备注说明（选填）</label>
              <textarea
                value={notes}

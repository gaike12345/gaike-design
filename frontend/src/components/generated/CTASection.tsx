...
export default function CTASection() {
  ...
  const fetchCta = async () => {
    try {
      const response = await configApi.getTable('cta_config');
      if (response.data?.data?.[0]) {
        setCta(response.data.data[0]);
      }
    } catch (error) {
  ...
  const defaultCta = {
    title: '准备好开启你的创意之旅了吗？',
    description: '无论你是需要专业服务，还是想加入学习社群，或者只是想聊聊你的创意想法，我们都在这里等你。',
    button1_text: '聊聊你的项目',
    ...
    <section className="py-20 bg-gradient-to-r from-[#4A5BFF]/10 to-[#7B3FF2]/10">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <Link
            to={data.button2_link || '/community'}

...
import { cfgNum } from '../config/siteConfig'
import { getRedis, isRedisReady } from '../cache/redis'
...
export const communityLimiter = rateLimit({
  ...
  legacyHeaders: false,
  validate: false,
  store: getStore('community'),
  keyGenerator(req: Request): string {
    const userId = (req as any).user?.userId
    if (userId) return `community:${userId}`
    return `community:${normalizeIp(req)}`
  },
  message: {
    ok: false,
    error: '操作过于频繁，请稍后再试',
...
export const passwordLimiter = rateLimit({
  ...
  legacyHeaders: false,
  validate: false,
  store: getStore('password'),
  keyGenerator(req: Request): string {
    const userId = (req as any).user?.userId
    if (userId) return `pwd:${userId}`
    return `pwd:${normalizeIp(req)}`
  },
  message: {
    ok: false,
    error: '密码操作过于频繁，请稍后再试',

...
// 通过 lib/llmRoute.ts 的 llmRouteJson / llmRouteText helper 统一封装：
//   - try/catch + fallback 结构
//   - demo 模式（无 API Key）下统一返回 placeholder: true（H17）
//   - 每个接口都通过 withGeneration 预扣额度（H13）
import { Router } from 'express'
import { withGeneration } from '../middleware/generation'
import { novelLimiter } from '../middleware/rate-limit'
import { authRequired } from '../middleware/auth'
import { llmRouteJson, llmRouteText } from '../lib/llmRoute'
...
// ==================== 去 AI 味清洗器（与 writingPrompt.ts 规则对齐）====================
// 设计目的：LLM（尤其 GLM-4-Flash）对负面指令的遵循力有限，单纯 prompt 无法 100% 保证
// 输出不含禁用词。此处做程序化兜底替换，确保续写正文严格符合去AI味规则。
// 规则来源：server/src/lib/writingPrompt.ts 第 14-40 行「去AI味规则」
// 替换原则：情态类删除（不引入新禁用词），动作/表情/心理类替换为身体反应或动作
const AI_TASTE_REPLACEMENTS: Array<{ from: RegExp; to: string; reason: string }> = [
  ...
  // 最毒句式：去掉"不是A，而是B"的对比结构，直接留 A
  { from: /不是([^，。！？\n]{1,15})，而是/g, to: '$1', reason: '最毒句式「不是A，而是B」' },
  // 破折号 / 长破折号：替换为逗号
  { from: /——|—|--/g, to: '，', reason: '破折号禁用' },
]

function sanitizeAiTaste(text: string): string {
  if (!text) return text
  let cleaned = text
  for (const rule of AI_TASTE_REPLACEMENTS) {
    cleaned = cleaned.replace(rule.from, rule.to)
  }
  // 清理因删除产生的多余标点（连续逗号、空逗号、行首逗号）
  cleaned = cleaned
    .replace(/，\s*，/g, '，')
    .replace(/，{2,}/g, '，')
    .replace(/^\s*，|，(?=[。！？；\n])/g, '')
...
router.post('/synopsis-options', withGeneration('novel', 500), llmRouteJson<SynopsisOptionsData>(
  '你是 Mank TV 的 AI 创作助手，擅长根据题材、受众、视角、篇幅等维度生成差异化的故事梗概方案。请返回 JSON：{ "options": [{ "id": "A"|"B"|"C", "title": string, "synopsis": string(100-200字), "tags": string[](3-5个) }] }，三个方案风格各异。不要包含其他说明文字。',
  (body) => {
...
router.post('/master-outline', withGeneration('novel', 500), llmRouteJson<MasterOutlineData>(
  '你是 Mank TV 的 AI 创作助手，擅长长篇小说的总纲架构设计。请根据主题、梗概、题材，返回 JSON：{ "premise": 核心设定, "theme": 主题, "volumes": [{ "name": 卷名, "summary": 卷摘要 }], "mainline": 主线脉络, "ending": 结局走向 }。卷数 3-5 卷。不要包含其他说明文字。',
  (body) => {
    const { topic } = body || {}
    return {
...
router.post('/character-relations', withGeneration('novel', 500), llmRouteJson<CharacterRelationsData>(
  '你是 Mank TV 的 AI 创作助手，擅长梳理小说角色之间的关系网络。请根据角色列表和主题，返回 JSON：{ "relations": [{ "from": 角色名, "to": 角色名, "type": 关系类型(如 朋友/敌人/师徒/恋人/宿敌), "desc": 关系描述 }] }。覆盖主要角色对。不要包含其他说明文字。',
  (body) => {
...
router.post('/volume-outline', withGeneration('novel', 500), llmRouteJson<VolumeOutlineData>(
  '你是 Mank TV 的 AI 创作助手，擅长分卷大纲设计。请根据卷名、主题、总纲，返回 JSON：{ "summary": 卷摘要, "chapters": [{ "title": 章名, "summary": 章摘要 }], "arc": 本卷弧光 }。章节数 5-10 章。不要包含其他说明文字。',
  (body) => {
...
router.post('/chapter-outline', withGeneration('novel', 500), llmRouteJson<ChapterOutlineData>(
  '你是 Mank TV 的 AI 创作助手，擅长章节细纲设计。请根据章名、卷摘要、主题，返回 JSON：{ "summary": 本章摘要, "scenes": [{ "location": 场景地点, "description": 场景描述, "dialogue": 关键对白, "mood": 情绪基调 }], "cliffhanger": 章末钩子 }。场景数 3-5 个。不要包含其他说明文字。',
  (body) => {
...
router.post('/continue-text', withGeneration('novel', 500), llmRouteText<{ content: string }>(
  '你是 Mank TV 的 AI 创作助手，擅长小说正文续写。任务：基于「当前正文」自然延续故事，字数贴近目标字数。\n'
  + '前置设定约束（必须严格遵循）：\n'
...
router.post('/continue-plot', withGeneration('novel', 500), llmRouteJson<ContinuePlotData>(
  '你是 Mank TV 的 AI 创作助手，擅长情节推演。请根据用户提供的文本和发展方向，返回 JSON：{ "development": 情节发展描述, "nextScene": 下一场景描述, "tension": 张力值(0-100整数) }。不要包含其他说明文字。',
  (body) => {
...
router.post('/book-title', withGeneration('novel', 200), llmRouteText<{ titles: string[] }>(
  '你是 Mank TV 的 AI 创作助手，擅长为小说起吸引人的书名。请根据主题、梗概、题材，生成 5-8 个风格各异的书名候选。直接输出书名，每行一个，不要编号、引号或其他说明。',
  (body) => {
...
router.post('/opening-line', withGeneration('novel', 200), llmRouteText<{ content: string }>(
  '你是 Mank TV 的 AI 创作助手，擅长为小说撰写引人入胜的开篇导语。请根据主题、梗概、风格，生成一段 50-150 字的导语，氛围感强、能勾起阅读欲望。直接输出导语文本，不要包含说明或标题。',
  (body) => {
    const { topic } = body || {}
    return {
...
router.post('/inspiration', withGeneration('novel', 500), llmRouteJson<InspirationData>(
  '你是 Mank TV 的 AI 创作助手，擅长脑洞创意生成。请根据关键词、题材，生成 3-5 个差异化的创意点子，返回 JSON：{ "ideas": [{ "title": 标题, "synopsis": 梗概(50-100字), "tags": 标签(2-4个) }] }。不要包含其他说明文字。',
  (body) => {
...
router.post('/smart-chat', withGeneration('novel', 200), llmRouteText<{ content: string }>(
  '你是 Mank TV 的 AI 创作助手「智能蛙」，擅长为小说创作者提供创作建议、剧情推演、设定答疑、卡文突破。请根据用户消息、上下文、小说信息，给出具体、可操作、有启发性的回复。直接输出回复文本，不要包含说明或前后缀。',
  (body) => {
    const { message } = body || {}
    return {
...
router.post('/outline', withGeneration('novel', 500), llmRouteJson<OutlineData>(
  '你是 Mank TV 的 AI 创作助手，擅长故事大纲设计(WR-01)。请根据主题、风格，返回 JSON：{ "theme": 主题, "acts": [{ "id": 序号, "name": 幕名, "summary": 幕摘要, "beats": 节拍列表(字符串数组) }], "characters": [{ "name": 角色名, "role": 角色定位, "arc": 角色弧光 }] }。幕数 3-5 幕，beats 每幕 3-6 个。不要包含其他说明文字。',
  (body) => {
...
router.post('/worldview', withGeneration('novel', 500), llmRouteJson<WorldviewData>(
  '你是 Mank TV 的 AI 创作助手，擅长世界观构建(WR-05)。请根据主题，返回 JSON：{ "name": 世界名, "genre": 题材类型, "geography": 地理, "history": 历史, "factions": [{ "name": 势力名, "desc": 描述, "stance": 立场 }], "rules": 规则数组, "culture": 文化, "conflicts": 核心冲突 }。factions 2-4 个，rules 3-6 条。不要包含其他说明文字。',
  (body) => {
    const { topic } = body || {}
    return {
      name: `${topic || '新'}世界`,
...
router.post('/lorebook', withGeneration('novel', 500), llmRouteJson<LorebookData>(
  '你是 Mank TV 的 AI 创作助手，擅长从文本中抽取设定词条(WR-12)。请根据源文本和已有词条，返回 JSON：{ "entries": [{ "key": 词条名, "category": 分类(人物/地点/物品/势力/概念/事件), "content": 内容描述, "aliases": 别名数组 }], "existing": 原样回传的已有词条数组 }。不要包含其他说明文字。',
  (body) => {
    const { existing } = body || {}
    return {
      entries: [
...
router.post('/prompt-helper', withGeneration('novel', 200), llmRouteText<{ prompt: string }>(
  '你是 Mank TV 的 AI 创作助手，擅长将中文自然语言描述转化为高质量的 AI 绘图 Prompt(WR-19)。请根据输入、风格、比例，生成一段英文 Prompt，包含主体、场景、光影、画风、构图、质量增强词。直接输出 Prompt 文本，不要包含说明或前后缀。',
  (body) => {
...
router.post('/enhance-prompt', withGeneration('novel', 200), llmRouteText<{ prompt: string }>(
...
router.post('/deepseek', withGeneration('novel', 500), llmRouteJson<DeepseekData>(
  '你是 Mank TV 的 AI 创作助手，擅长多路径剧情推演(WR-15)。请根据情境和候选选项，返回 JSON：{ "analysis": 局势分析, "paths": [{ "id": 序号, "title": 路径名, "development": 发展推演, "consequence": 后果, "drama": 戏剧性(0-100整数) }], "recommendation": 推荐建议 }。paths 3-5 条。不要包含其他说明文字。',
  (body) => {
...
router.post('/style-clone', withGeneration('novel', 500), llmRouteText<{ content: string; styleFeatures: string }>(
  '你是 Mank TV 的 AI 创作助手，擅长模仿文风写作(WR-14)。请先仔细分析样本文风的用词、句式、节奏、修辞特点，然后用相同的文风创作一段符合主题的内容。输出格式：先输出创作内容，然后另起一行输出「文风特征：」开头，后接 2-3 条文风特点总结。',
  (body) => {
    const { topic } = body || {}
    return {
...
router.post('/ai-erase', withGeneration('novel', 200), llmRouteText<{ content: string; before: string }>(
  '你是 Mank TV 的 AI 创作助手，擅长消除文本的 AI 生成痕迹(WR-18)。请根据输入文本和强度(light/medium/heavy)，调整用词、句式、节奏，去除模板化表达、"首先其次最后"等套路、过度对仗与排比，使其更自然、更像人类写作。直接输出修改后的文本，不要包含说明或前后缀。',
  (body) => {
    const { input } = body || {}
    return {
      content: input || '（无输入文本）',
...
// 20. POST /script — 脚本生成
router.post('/script', withGeneration('novel', 500), llmRouteJson<ScriptData>(
  '你是 Mank TV 的 AI 创作助手，擅长漫剧脚本生成。请根据主题、风格、分镜数，返回 JSON：{ "title": 标题, "synopsis": 简介, "characters": [{ "name": 角色名, "desc": 描述, "role": 定位 }], "scenes": [{ "id": 序号, "location": 地点, "shot": 镜头描述, "description": 场景描述, "dialogue": 对白, "mood": 情绪 }], "genre": 题材 }。scenes 数量按 shots 参数(默认 6)。不要包含其他说明文字。',
  (body) => {
...
router.post('/storyboard', withGeneration('novel', 500), llmRouteJson<ScriptScene[]>(
  '你是 Mank TV 的 AI 创作助手，擅长将长脚本拆解为分镜列表。请根据脚本和目标数量，返回 JSON 数组：[{ "id": 序号, "location": 地点, "shot": 镜头描述, "description": 场景描述, "dialogue": 对白, "mood": 情绪 }]。按目标数量(count 参数，默认 6)拆分。不要包含其他说明文字。',
  (body) => {
    const { count } = body || {}
...
router.post('/dialogue', withGeneration('novel', 500), llmRouteJson<DialogueLine[]>(
  '你是 Mank TV 的 AI 创作助手，擅长场景对白创作。请根据角色列表、场景、提示，返回 JSON 数组：[{ "character": 角色名, "line": 台词, "emotion": 情绪(如 平静/激动/愤怒/悲伤/惊喜/恐惧) }]。每角色至少 2 句对白，对话自然、有冲突与节奏。不要包含其他说明文字。',
  (body) => {

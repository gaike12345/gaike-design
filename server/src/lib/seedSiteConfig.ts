...
// ====== 内容安全：默认敏感词库（6 大违规类别，网信办审查重点） ======
// 注：为示例占位词库，实际部署应结合网信办违规内容指引与业务场景持续扩充。
// 管理员可在后台「内容审核」面板按类别可视化增删，修改后即时生效（走 siteConfig 缓存）。
export const DEFAULT_SENSITIVE_WORDS: Record<string, string[]> = {
  political: [
    // 中文
...
export const DEFAULT_ITEMS: DefaultItem[] = [
  ...
  { group: 'safety', key: 'safety.moderation_level', defaultValue: 'standard', controlType: 'select', label: '内容审核严格度', sort: 2, icon: 'ShieldAlert',
    description: '宽松=仅拦截政治/暴恐/色情/毒品等高风险类；标准=额外拦截赌博/侮辱；严格=全类别拦截，面向未成年人场景。',
    config: { options: [
      { label: '宽松（仅拦截高风险违法违规）', value: 'loose' },
      { label: '标准（默认）', value: 'standard' },
      { label: '严格（面向未成年人）', value: 'strict' },
    ] } },
  // 敏感词库：JSON 存储，后台「内容审核」面板提供按类别可视化增删编辑器
  { group: 'safety', key: 'safety.moderation_sensitive_words', defaultValue: DEFAULT_SENSITIVE_WORDS, controlType: 'textarea', label: '敏感词库（JSON）', sort: 3, icon: 'ListFilter',
    description: '6 大违规类别词库（political/violence/porn/gambling/drug/insult）。建议在「内容审核」面板的可视化编辑器中增删，此处为 JSON 原文备份。修改后即时生效。', config: { rows: 12 } },

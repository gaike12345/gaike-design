...
import { BusinessError, ForbiddenError, SystemError } from '../../mank-common/errors'
...
export async function createWechatScanScene(): Promise<{
  ...
  // 未配置微信公众号 → 微信登录不可用
  if (!appId || !appSecret) {
    throw new BusinessError('微信登录暂未开放，请使用密码登录')
  }
  ...
  try {
    ...
    if (!tokenData.access_token) {
      throw new SystemError(tokenData.errmsg || '获取 access_token 失败')
    }
    ...
    if (!qrData.ticket) {
      throw new SystemError(qrData.errmsg || '生成二维码失败')
    }
    ...
    throw new SystemError('微信登录暂时不可用，请稍后重试')
  }
}
...
async function getOrCreateWechatUser(openId: string, userInfo?: { nickname?: string; avatar?: string }) {
  ...
  if (!user) {
    ...
    throw new ForbiddenError('账号已被关闭，请联系管理员')
  }

...
async function submitTask(params: ImageGenerateParams): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new SystemError(`Dashscope 提交任务失败 ${res.status}: ${text.slice(0, 200)}`)
  }

  const data = await res.json() as any
  const taskId = data?.output?.task_id || data?.task_id
  if (!taskId) throw new SystemError('Dashscope 返回无 task_id')
  return taskId
}
...
async function waitForResult(taskId: string): Promise<string> {
  ...
  while (Date.now() - startTime < POLL_TIMEOUT_MS) {
    ...
    if (status === 'SUCCEEDED') {
      ...
      throw new SystemError('Dashscope 生成成功但无图片 URL')
    }

    if (status === 'FAILED') {
      const msg = data?.output?.message || data?.message || '未知错误'
      throw new SystemError(`Dashscope 生成失败: ${msg}`)
    }

    // PENDING / RUNNING -> 继续等
  }

  throw new SystemError('Dashscope 生成超时（60 秒）')
}
...
export const dashscopeProvider: ImageProvider = {
  ...
  async textToImage(params: ImageGenerateParams): Promise<ImageResult> {
    if (!API_KEY) {
      throw new BusinessError('Dashscope 未配置 API Key（DASHSCOPE_API_KEY）')
    }

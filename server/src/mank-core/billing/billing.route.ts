 ...
 *     tags: [充值订阅]
 *     summary: 获取会员套餐列表
 *     description: 获取所有会员套餐列表及计费配置（公开接口，无需登录）
 *     security: []
 *     responses:
 *       200:
 *         description: 会员套餐列表
 ...
 *                     note:
 *                       type: string
 *                       example: "会员套餐预览，正式购买请联系管理员"
 */
router.get('/plans', async (_req: Request, res: Response, next: NextFunction) => {
  logger.info('CTRL_BILLING_PLANS_GET', {})
  try {
 ...
 *     tags: [充值订阅]
 *     summary: 获取充值套餐列表
 *     description: 获取所有充值套餐列表及货币信息（公开接口，无需登录）
 *     security: []
 *     responses:
 *       200:
 *         description: 充值套餐列表
 ...
 *     tags: [充值订阅]
 *     summary: 创建充值订单
 *     description: 根据充值套餐 ID 创建充值订单（当前为模拟支付模式，返回占位响应）
 *     security: [{ BearerAuth: [] }]
 *     requestBody:
 *       required: true
 *       content:
...
router.post('/recharge', async (req: Request, res: Response, next: NextFunction) => {
  logger.info('CTRL_BILLING_RECHARGE_POST', { userId: req.user!.userId, packageId: req.body.packageId })
  try {
 ...
 *     tags: [充值订阅]
 *     summary: 创建或升级会员订阅
 *     description: 根据套餐 ID 创建订阅订单（免费版直接切换，付费版创建订单）
 *     security: [{ BearerAuth: [] }]
 *     requestBody:
 *       required: true
 *       content:
...
router.post('/subscribe', async (req: Request, res: Response, next: NextFunction) => {
  logger.info('CTRL_BILLING_SUBSCRIBE_POST', { userId: req.user!.userId, planId: req.body.planId })
  try {
    const userId = req.user!.userId
    ...
    res.json({
      ...
      payUrl: `https://pay.mantv.cn/mock/${order.id}`,
      payMockUrl: `/api/billing/pay/${order.id}`,
      message: `订阅 ${plan.name} 订单已创建，正式支付请联系管理员`,
 ...
 *     tags: [充值订阅]
 *     summary: 模拟支付完成
 *     description: 模拟支付回调接口（仅非生产环境可用，生产环境须由支付网关签名回调驱动）
 *     security: [{ BearerAuth: [] }]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *                     message:
 *                       type: string
 *                       example: "模拟支付成功，额度已到账（生产环境请使用真实支付网关回调）"
 *       400:
...
router.post('/pay/:id', async (req: Request, res: Response, next: NextFunction) => {
  logger.info('CTRL_BILLING_PAY_POST', { userId: req.user!.userId, orderId: req.params.id })
  try {
 ...
 *     tags: [充值订阅]
 *     summary: 获取订单列表
 *     description: 分页查询当前用户的支付订单列表，可按状态筛选
 *     security: [{ BearerAuth: [] }]
 *     parameters:
 *       - in: query
 *         name: status
...
router.get('/orders', async (req: Request, res: Response, next: NextFunction) => {
  logger.info('CTRL_BILLING_ORDERS_GET', { userId: req.user!.userId, status: req.query.status, page: req.query.page, pageSize: req.query.pageSize })
  try {
    const userId = req.user!.userId

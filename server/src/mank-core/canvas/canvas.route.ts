...
import { callLlm, callLlmJson } from '../llm/llmProvider'
import { moderateText, recordViolation, checkInputModeration } from '../moderation/moderation'
 ...
 *     tags: [创作画布]
 *     summary: 脚本分镜生成
 *     description: 根据文本提示词生成可执行的脚本分镜列表，包含场景、景别、镜头运动、对白和图像提示词。LLM 未配置时返回本地占位分镜。
 *     security: [{ BearerAuth: [] }]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *                 placeholder:
 *                   type: boolean
 *                   description: true 表示本地占位分镜（LLM 未配置）
 *       400:
 *         description: 参数错误
 *         content:
 *           application/json:
...
router.post('/script', withGeneration('novel', 500), async (req, res) => {
  logger.info('CTRL_CANVAS_SCRIPT', { userId: (req as any).user?.userId, prompt: req.body?.prompt?.slice(0, 50) })
  ...
  const sys = `你是 Man TV 的 AI 分镜导演，擅长将文本创意拆解为可执行的镜头脚本。请根据用户输入，生成 ${shotCount} 个分镜。返回 JSON 格式：
{
  "title": "作品标题",
  "synopsis": "一句话简介",
  "shots": [
 ...
 *     tags: [创作画布]
 *     summary: 画布服务状态
 *     description: 返回创作画布的可用节点类型、模型列表及 LLM 配置状态。
 *     security: []
 *     responses:
 *       200:
 *         description: 服务正常
 ...
 *                 llmConfigured:
 *                   type: boolean
 *                   description: LLM 是否已配置
 */
router.get('/status', (_req, res) => {
  logger.info('CTRL_CANVAS_STATUS', {})
  res.json({
    ok: true,
    canvas: 'unified',
